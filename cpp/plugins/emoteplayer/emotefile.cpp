    ...
    else
    {
             ...
             (frame->type == 2 && nextframe->type == 2))) // 存在下一帧则对关键帧进行插值
        {
            ...
            if (std::isinf(frame->coordY))
                frame->coordY = lim.width - lim.originY;
            if (std::isinf(nextframe->coordX))
                nextframe->coordX = lim.height - lim.originX;
...
public:
    ...
    void update(float deltaTime)
    {
        ...
        currentWindPower = windPowerMin + (windPowerMax - windPowerMin) *
...
public:
    ...
    void initializeHairPhysics(const std::vector<HairSegment>& segs,
                               ...
                               const std::string& ud)
    {
        hairPhysics.initialize(segs, bRate, grav, fricX, fricY, bendSpd, bendVol, ofs, vBound, lr,
                               lrm, ud);
    }
    ...
    void initializeBustPhysics(const std::string& baseLayer,
                               ...
                               const std::string& ud)
    {
        bustPhysics.initialize(baseLayer, enable, fric, grav, op, p, pv, ofs, spr, scl_x, scl_y, lr,
                               ud);
    }
...
public:
    void initialize()
    {
        ...
        physicsSystem.initializeHairPhysics(hairSegments, 0.00371093745f, 0.6f, 0.046875f, 0.09375f,
                                            0.2f, 3.0f, -161.684357f, true, "hair_LR_front",
                                            "hair_LR_M_front", "hair_UD_front");
...
SmoothPeriodicRandom::SmoothPeriodicRandom() : noise_seed(std::rand() / float(RAND_MAX) * 1000.0f)
{
}
...
BreastJiggleSimulator::SpringDamper::SpringDamper(float stiff, float damp, float m)
  : position(0),
    velocity(0),
    target(0),
    stiffness(stiff),
